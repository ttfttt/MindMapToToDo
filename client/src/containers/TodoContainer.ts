...
export function getTodo(id: string): Promise<TodoInfo> {
    return axios.request<TodoInfo>({
        method: "get",
        url: process.env.REACT_APP_REST_API_HOST + `/api/todo/${id}`
    }).then(res => res.data);
};
...
export function addTodo(todo: TodoInfo): Promise<TodoInfo> {
    return axios.request<TodoInfo>({
        method: "post",
        url: process.env.REACT_APP_REST_API_HOST + "/api/todo",
        data: todo
    })
        .then(res => res.data)
};
...
export function updateTodo(todo: TodoInfo): Promise<TodoInfo> {
    return axios.request<TodoInfo>({
        method: "put",
        url: process.env.REACT_APP_REST_API_HOST + `/api/todo/${todo.id}`,
        data: todo
    }).then(res => res.data);
};
